...
* На других платформах ключ шифруется алгоритмом AES-256: для каждой записи генерируется случайная соль, которая хранится вместе с шифротекстом. Ключ шифрования выводится из мастер-пароля по PBKDF2 (SHA-256, 100k итераций).
...
* Значения, сохранённые предыдущими версиями (с детерминированной солью), продолжают корректно расшифровываться.
...
* Добавлены модульные тесты `SecureSettingsTests`, проверяющие корректность шифрования/расшифровки, уникальность шифротекста за счёт случайной соли и устойчивость к неверному мастер-паролю.
* Для Linux/macOS тесты автоматически выставляют `PLAYCANVAS_MASTER_PASSWORD=UnitTestSecret!` и восстанавливают предыдущее значение переменной после выполнения.
