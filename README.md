...
- **Сжатие**: Basis Universal (ETC1S, UASTC) через ktx с поддержкой KTX2
...
### Внешние зависимости
- **ktx** v4.3.0+ из KTX-Software для конвертации текстур в KTX2
  - Установка Windows: `winget install KhronosGroup.KTX-Software`
  - Проверка: `ktx --version`
...
**Симптомы**:
```
warning NU1902: У пакета "SixLabors.ImageSharp" 3.1.7 есть известная уязвимость
GHSA-rxmq-m78w-7wmc (уровень серьезности: средний)
```

**Статус**:
Предупреждение NU1902 для ImageSharp 3.1.7 можно безопасно игнорировать:
- Это **известная проблема** с низким риском (средний уровень серьезности)
- Уязвимость касается специфических сценариев обработки изображений
- Версия 3.1.7 - **текущая последняя стабильная** версия
- Уязвимость NU1903 (высокая) была исправлена в 3.1.7

**Если хотите подавить предупреждение**:
Добавьте в `.csproj`:
```xml
<PropertyGroup>
...
**Альтернатива**: Дождитесь выхода ImageSharp 3.1.8+ с исправлением
...
> **⚠️ Важно**: Файл `PlayCanvasImporterWindow.cs` **исключен из сборки** WPF проекта, так как он использует Unity API. Он предназначен исключительно для использования в Unity Editor.

### Использование

1. Скопируйте `PlayCanvasImporterWindow.cs` из корня проекта в папку `Assets/Editor/` вашего Unity проекта
2. В Unity Editor откройте: **Window** → **PlayCanvas Importer**
3. Укажите путь к JSON файлу сцены PlayCanvas
4. Нажмите **Import Scene**

**Что делает скрипт**:
- Создает иерархию GameObjects из JSON сцены PlayCanvas
- Конвертирует координаты и трансформы (position, rotation, scale)
- Устанавливает корректную родительскую структуру объектов

**Примечание**: Скрипт работает только в Unity Editor (не в runtime)
