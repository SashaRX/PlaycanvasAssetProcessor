...
public class ProjectAssetService {
    ...
    private async Task<ModelResource?> ProcessModelAssetAsync(
        ...
        string resourcePath = localCacheService.SanitizePath(
            localCacheService.GetResourcePath(projectsRoot, projectName, folderPaths, cleanFileName, parentId));
        string modelName = cleanFileName.Split('.')[0];
    ...
    private async Task<MaterialResource?> ProcessMaterialAssetAsync(
        ...
        string rawName = asset["name"]?.ToString() ?? "Unknown";
        string cleanName = localCacheService.SanitizePath(rawName);
        int? parentId = asset["parent"]?.Type == JTokenType.Integer ? (int?)asset["parent"] : null;
        string materialPath = localCacheService.SanitizePath(
            localCacheService.GetResourcePath(projectsRoot, projectName, folderPaths, $"{cleanName}.json", parentId));
